...
interface CostBreakdownCarouselProps {
  className?: string
}

const PAGES = [1, 3, 4, 5, 6, 7, 8]

export function CostBreakdownCarousel({ className = '' }: CostBreakdownCarouselProps) {
  ...
  const pages = useMemo(
    () =>
      PAGES.map((num) => ({
        id: num,
        src: `/docs/cost-breakdown-analysis-report/cost-breakdown-analysis-page-0${num}.png`,
        alt: `Cost Breakdown Analysis Report - Page ${num}`,
      })),
    []
  )
